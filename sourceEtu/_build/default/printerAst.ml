...
struct
  ...
  let rec string_of_instruction i =
    ...
    | Affectation (n,e) ->  "Affectation  : "^(string_of_expression e)^"\n"
  ...
  let string_of_fonction (Fonction(t,n,lp,li)) = (string_of_type t)^" "^n^" ("^((List.fold_right (fun ((t,b),n) tq -> (string_of_type t)^"*"^(string_of_bool b)^" "^n^" "^tq) lp ""))^") = \n"^
