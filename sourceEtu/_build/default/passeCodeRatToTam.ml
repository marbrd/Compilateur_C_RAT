...
let rec analyse_code_affectable aff lec = 
  ...
  | AstType.Ident info ->
    begin
      ...
      | InfoVar (_,(t,b),dep,reg) -> 
        let len = (getTaille (t,false)) in
        if b then
          if not lec then (
            load 1 dep reg
            ^storei len 
...
let rec analyse_code_expression e = 
  ...
  | AstType.New t -> 
    loadl_int (getTaille (t,false))
    ^ subr "MAlloc"
  | AstType.Adresse info -> 
    begin
      match info_ast_to_info info with
      | InfoVar(_,(_,b),dep,reg) -> 
        if not b then loada dep reg
        else load 1 dep reg
      | _ -> failwith "erreur interne"
    end 
  | AstType.Reference _ -> ""
...
let rec analyse_code_instruction i =
  ...
  | AstPlacement.FinVoid tailleParam -> return 0 tailleParam
