...
type expression =
  ...
  (*l'adresse d'un identifiant*)
  | Adresse of string
  (*definition d'un pointeur de type typ*)
  | New of typ
  (* Booléen *)
  | Booleen of bool
  (* Entier *)
  ...
  (*pour la definiton d'un pointeur null*)
  | Null
  (* reference *)
  ...
  | Evaleur of string
and affectable =
  (*le nom de l'identifiant d'un affectable*)
  | Ident of string
  (*pour déreferencer un affectable*)
  | Deref of affectable
...
and instruction =
  ...
  (* Affectation d'un affectable et la nouvelle valeur affectée *)
...
type enums = AstTds.enums

(* Opérateurs unaires de Rat - résolution de la surcharge *)
type unaire = Numerateur | Denominateur

(* Opérateurs binaires existants dans Rat - résolution de la surcharge *)
type binaire = Fraction | PlusInt | PlusRat | MultInt | MultRat | EquInt | EquBool | Inf | EquTenum
